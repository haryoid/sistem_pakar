	...
	if(empty($kode) or empty($nama)){
		...
		if($action=='add'){
			if(mysql_num_rows(mysql_query("select * from penyakit where kode='".$kode."'"))>0){
				...
				$q="insert into penyakit(kode, nama) values('".$kode."', '".$nama."', '".$notes."')";
				mysql_query($q);
...
<table class="table">
  ...
  <tr>
	...
	<tr>
	<td>Keterangan<span class="required">*</span> </td>
	<td><input name="notes" type="text" size="40" value="<?php echo $notes;?>" class="m-wrap large"></td>
  </tr>
  <tr>
	<td></td>
